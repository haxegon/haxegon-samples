...
class Astar {
	...
	public static function getnextmove():Int {
		...
		*/
		if (!pathexists()) {
			...
			if (pathx[2] > pathx[1]) return Help.RIGHT;
			if (pathx[2] < pathx[1]) return Help.LEFT;
			if (pathy[2] > pathy[1]) return Help.DOWN;
			if (pathy[2] < pathy[1]) return Help.UP;
		}
		return Help.NODIRECTION;
	}
	...
	public static function setmapcollision():Void {
		...
		//	World.mapchanged = false;
			...
			//trace("Setting map collision for A*, took " + (flash.Lib.getTimer() - starttime) + "ms");
		//}
	}
	
	public static function setcollidepoint(x:Int, y:Int):Void {
		if (Help.inbox(x, y, 0, 0, mapwidth, mapheight)) contents[x + vmult[y]] = 1;
	}
