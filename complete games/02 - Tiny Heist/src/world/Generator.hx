...
class Generator {
	...
	public static function placelater(x:Int, y:Int, thing:String, type:String):Void {
		...
		thingstoplace.push(thing);
		thingstoplace_type.push(type);
	}
	...
	public static function placethethings():Void {
		if(!rejectmap){
			for (i in 0 ... thingstoplace.length) {
				Obj.createentity(xplace[i], yplace[i], thingstoplace[i], thingstoplace_type[i], "preplaced");
			}
		}
	...
	public static function getintersectionpoint(x1:Int, y1:Int, dir1:Int, x2:Int, y2:Int, dir2:Int):Bool {
		...
		if (dir1 == Direction.clockwise(dir2) || dir1 == Direction.anticlockwise(dir2)) {
			...
			}else if (dir1 == Direction.UP || dir1 == Direction.DOWN) {
				tpoint.setTo(x1, y2);
				if (getdir(x1, y1) == dir1 && getdir(x2, y2) == dir2) {
					return true;
				}
			...
			if (numrooms > b && numrooms > a) {
				...
				if (roomoverlap_vertical(a, b)) {
					if (rooms[a].x < rooms[b].x) {
						rooms[a].facing = Direction.RIGHT;
						rooms[b].facing = Direction.LEFT;
					}else {
						rooms[a].facing = Direction.LEFT;
						rooms[b].facing = Direction.RIGHT;
					}
				}else if (roomoverlap_horizontal(a, b)) {
					if (rooms[a].y < rooms[b].y) {
						rooms[a].facing = Direction.DOWN;
						rooms[b].facing = Direction.UP;
					}else {
						rooms[a].facing = Direction.UP;
						rooms[b].facing = Direction.DOWN;
					}
				}else {
				...
				if (getintersectionpoint(tx1, ty1, rooms[a].facing, tx2, ty2, rooms[b].facing)) {
					...
					if (rooms[a].facing == Direction.opposite(rooms[b].facing)) {
						if (rooms[a].facing == Direction.LEFT || rooms[a].facing == Direction.RIGHT) {
							tr = Std.int((tx1 + tx2) / 2);
							addline(tx1, ty1, tr, ty1);
							addline(tr, ty1, tr, ty2);
							addline(tx2, ty2, tr, ty2);
						}else if (rooms[a].facing == Direction.UP || rooms[a].facing == Direction.DOWN) {
	...
	public static function setup_randomplacementlist():Void {
		...
		for (k in 0 ... numrooms) {
			if (S.isinstring(rooms[k].category, "exit") || 
					S.isinstring(rooms[k].category, "ripple") ||
					S.isinstring(rooms[k].category, "kingpin")) {
	...
	public static function generate(currentblueprint:String, regenerate:Bool = false):Void {
		...
		if (rejectmap) {
			...
			numrejections++;
			generate(currentblueprint, true);
		}
	}
	...
	public static var connectionsort:Array<Int> = [];
	...
	public static var thingstoplace:Array<String> = [];
	public static var thingstoplace_type:Array<String> = [];
	public static var xplace:Array<Int> = [];
	public static var yplace:Array<Int> = [];
}